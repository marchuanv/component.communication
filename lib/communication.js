...
const CommunicationPriority = require("./communication.priority.js");
...
Communication.prototype.handle = function({ callback, priority }) {
    if (typeof callback !== "function") {
        return await logging.write("component.communication", `unable to handle communcation from ${this.host}:${this.port}, callback is not a function`);
    }
    if (priority instanceof CommunicationPriority) {
        return await logging.write("component.communication", `unable to handle communcation from ${this.host}:${this.port}, priority is not of type CommunicationPriority`);
    }
    this.requestCallbacks.push({ priority, callback, response: {
        statusCode: 500,
