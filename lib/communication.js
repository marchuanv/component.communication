            ...
            req.on('error', async (error) => {
                ...
                if (message.retryCount < 4){
                    ...
                    const responseMessage = new CommunicationResponseMessage({
                        ...
                        headers,
                        body: error,
                        status: CommunicationStatus[500]
                    });
                    await responseMessage.validate();
                    await resolve(responseMessage);
                }
...
Communication.prototype.receive = function(callback) {
    if (this.lock){
        ...
        host.on("error", async (hostError) => {
            if (newHost.host){
                dns.lookup(config.component.host, async (dnsErr) => {
                    if (dnsErr){
                        return logging.write("component.communication", `error hosting on ${config.component.host}:${config.component.port}, error: ${dnsErr.message}`);
                    }
                });
            } else {
                return logging.write("component.communication", `error hosting on ${config.component.host}:${config.component.port}, error: ${hostError.message}`);
            }
        });
        host.on("listening", async () => {
            await logging.write("component.communication",`listening on ${config.component.host}:${config.component.port}`);
        });
        this.lock = false;
    }
};
