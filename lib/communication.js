        ...
        return new Promise((resolve, reject) => {
            ...
            req.on('error', async (error) => {
                ...
                if (retryCount <= 3){
                    ...
                    const errorMessage = new CommunicationIncomingMessage({
                        Id: message.Id,
                        headers: req.headers,
                        body: error,
                        statusCode: 500,
        ...
        host.on("request", (request, response) => {
            const id = setInterval( async () => {
                ...
                request.on('end', async () => {
                    ...
                    const incomingMessage = new CommunicationIncomingMessage({ Id: request.url.replace("/",""), headers: request.headers, body: request.body });
