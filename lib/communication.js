        ...
        host.on("request", (request, response) => {
            const id = setInterval( async () => {
                ...
                request.on('end', async () => {
                    ...
                    const incomingMessage = new CommunicationMessage({ Id: request.url.replace("/",""), headers: request.headers, body: request.body, statusCode: 200, statusMessage: "Success"});
                    incomingMessage.validate();
                    const outgoingMessage = await callback(incomingMessage);
                    if (outgoingMessage instanceof CommunicationMessage) {
