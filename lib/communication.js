...
Communication.prototype.send = function(message) {
    if (message instanceof CommunicationIncomingMessage) {
        message.validate();
        let { headers, retryCount } = message;
        return new Promise((resolve, reject) => {
            const { host, port } = config.component;
            const requestUrl = `${host}:${port}/${message.Id}`;
            ...
            const req = http.request({ host, port, path:`/${message.Id}`, method: "POST", timeout: 30000, headers }, (response) => {
                ...
                response.on('end',async () => {
                    ...
                    const receivedMessage = new CommunicationIncomingMessage({
                        Id: message.Id,
                    ...
                    if (receivedMessage instanceof CommunicationIncomingMessage) {
                        receivedMessage.validate();
                        await resolve(receivedMessage);
                    } else {
                        await reject(new Error(`recieved message is not of type: CommunicationIncomingMessage`));
                    }
                });
            });
            req.on('error', async (error) => {
                ...
                if (retryCount <= 3){
                    ...
                    const errorMessage = new CommunicationOutgoingMessage({
                        Id: message.Id,
                        headers: req.headers,
                        body: error,
                        statusCode: 500,
        ...
        throw new Error("message parameter is not of type: CommunicationIncomingMessage");
    }
};
        ...
        host.on("request", (request, response) => {
            const id = setInterval( async () => {
                ...
                request.on('end', async () => {
                    ...
                    const incomingMessage = new CommunicationIncomingMessage({ Id: request.url.replace("/",""), headers: request.headers, body: request.body, statusCode: 200, statusMessage: "Success"});
                    if (incomingMessage instanceof CommunicationIncomingMessage) {
                        incomingMessage.validate();
                        const outgoingMessage = await callback(incomingMessage);
                        if (outgoingMessage instanceof CommunicationOutgoingMessage) {
                            outgoingMessage.validate();
                            outgoingMessage.headers["Content-Length"] = Buffer.byteLength(outgoingMessage.toString());
