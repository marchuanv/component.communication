...
Communication.prototype.send = ({ componentName ,method, headers, data, retryCount = 1 }) => {
    return new Promise((resolve) => {
        ...
        const req = http.request({ host, port, path:`/${componentName}`, method, timeout: 30000, headers }, (response) => {
        ...
        req.on('error', async (error) => {
            ...
            if (retryCount <= 3){
                const res = await this.send({ path:`/${componentName}`, method, headers, data, retryCount });
                await resolve(res);
            } else {
...
Communication.prototype.receive = () => {
    return new Promise((resolve) => {
        if (this.lock){
            setTimeout(async () => {
