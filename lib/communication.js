        ...
        host.on("request", (request, response) => {
            const id = setInterval( async () => {
                ...
                request.on('end', async () => {
                    ...
                    const incomingMessage = new CommunicationMessage({ headers: request.headers, body: request.body, statusCode: 200, statusMessage: "Success"});
                    ...
                    const responseData = utils.getJSONString(outgoingMessage);
                    data.message.headers["Content-Length"] = Buffer.byteLength(responseData);
                    response.writeHead(outgoingMessage.statusCode, outgoingMessage.statusMessage, outgoingMessage.headers ).end(responseData);
                    this.lock = false;
                });
            },1000);
        });
