...
Communication.prototype.send = function({ component, message }) {
    const { headers, text, retryCount = 1 } = message;
    return new Promise((resolve, reject) => {
        ...
        req.on('error', async (error) => {
            ...
            if (retryCount <= 3){
                const res = await this.send(component, { headers, text, retryCount });
                await resolve(res);
            } else {
