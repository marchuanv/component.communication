...
function RequestHandler({ priority, callback }) {
    ...
    Object.defineProperty(this, 'handle', { writable: false, value: async function({ url, requestHeaders, requestBody }) {
        ...
        if(isPreflight) {
            ...
            const requestMessage = new RequestMessage({ Id: url.replace("/",""), headers: requestHeaders, body: requestBody });
            let bodyCollate = [];
            let hasSelectedHeaders = false;
            try {
                await requestMessage.validate();
                let responseMessages = await this.callback(requestMessage);
                if (responseMessages) {
                    ...
                    for(const responseMessage of responseMessages) {
                        if (responseMessage instanceof ResponseMessage) {
                            ...
                            this.response.body = "response from request handler callback is not a valid response message";
                        }
                    };
                } else {
                    this.response.statusCode = 500;
                    this.response.statusMessage = messageStatus.getMessage(500);
                    this.response.headers = {};
                    this.response.body = "no response message from request handler callback";
                }
            } catch (err) {
