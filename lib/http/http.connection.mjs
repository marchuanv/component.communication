        ...
        const req = request({
            ...
            response.on('end', async () => {
                ...
                try {
                    ...
                    responseData = new Error('could not parse request body to json');
                }
                if (!(responseData instanceof Error)) {
                    if (response.statusCode >= 200 && response.statusCode < 300) {
                        responseData = { headers, data: responseData }
                    } else {
                        responseData = new Error(body);
                    }
                }
                raiseEvent.call(this, responseData);
            });
        });
                ...
                request.on('end', () => {
                    ...
                    if (body && headers) {
                        try {
                            ...
                            responseData = 'message received and is valid';
                        } catch {
                            statusCode = 400;
                        ...
                        statusCode = 400;
                        statusMessage = '400 Bad Request';
                        responseData = new Error('no request body');
                    }
                    raiseEvent.call(this, responseData);
                    response.writeHead(statusCode, statusMessage, resHeaders).end(responseData);
                });
            });
        });
        server.on("listening", () => {
